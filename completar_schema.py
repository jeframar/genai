...
def extraer_texto_seccion_raw(client: genai.Client, file_ref, nombre_seccion: str, ruta_contexto: list[str]) -> str:
...
TU TAREA:
Extrae el texto crudo y literal completo correspondiente a la sección: "{nombre_seccion}".
...
6. FORMATO DE SALIDA: Devuelve exclusivamente un objeto JSON válido con la clave "texto".

EJEMPLO DE SALIDA:
{{
  "texto": "Texto literal del primer párrafo..."
}}"""
...
def extraer_texto_seccion_con_reintentos(
    ...
    for intento in range(1, intentos_totales + 1):
        try:
            return extraer_texto_seccion_raw(client, file_ref, nombre_seccion, ruta_contexto)
...
def main():
    ...
    # Solamente generamos y actualizamos schema_completado.json
